...
const validarAsunto = () => {
    ...
    if (!asuntoValue) {
        ...
        asuntoError.style.display = 'block'; // visible mensaje de error
        asunto.classList.add('is-invalid');
    } else {
        asuntoError.style.display = 'none'; // oculta el mensaje de error
        asunto.classList.remove('is-invalid'); // elimina el estilo de error
        asunto.classList.add('is-valid'); // agrega estilo de éxito
    }
};
...
document.getElementById('formularioContacto').addEventListener('submit', (event) => {
    ...
    if (camposInvalidos.length > 0) {
        esValido = false; // Si hay campos invalidos, no se envía el formulario
        document.getElementById('mensajeErrorGlobal').style.display = 'block'; // Muestra el mensaje de error global
    } else {
        document.getElementById('mensajeErrorGlobal').style.display = 'none'; // Oculta el mensaje de error global
    }

    // Si todo es válido, enviar el formulario
    if (esValido) {
        document.getElementById('mensajeRespuesta').style.display = 'block'; // Muestra mensaje de éxito
        setTimeout(() => {
            document.getElementById('formularioContacto').reset(); // Reinicia el formulario
            document.getElementById('mensajeRespuesta').style.display = 'none'; // Oculta el mensaje de éxito
        }, 3000); // Espera 3 segundos antes de limpiar el formulario
    }
});
